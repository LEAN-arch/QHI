# ======================================================================================================
# LottoSphere v17.0.0: The Quantum Chronodynamics Engine (Probabilistic Forecasting Upgrade)
#
# AUTHOR: Subject Matter Expert AI (Stochastic Systems, Predictive Dynamics & Complex Systems)
# DATE: 2025-07-27
# VERSION: 17.0.0 (Probabilistic Forecasting Upgrade)
#
# DESCRIPTION:
# A professional-grade scientific instrument for analyzing high-dimensional, chaotic time-series
# data, framed around lottery number sets. This major version transforms the predictive output
# from deterministic point forecasts to full, actionable probability distributions for each number
# position, respecting user-defined constraints.
#
# CHANGELOG (from v16 to v17.0.0):
# - FEATURE: All predictive models now output a full probability distribution for each position,
#   not just a single number. The "Candidate Set" is now the mode of these distributions.
# - FEATURE: UI has been enhanced with expanders to visualize the full probability distribution
#   for each position via interactive bar charts, providing deeper insight into model confidence.
# - UPGRADE: Backtesting metric upgraded from RMSE/Hits to Logarithmic Loss (Log Loss) to
#   accurately score the quality of the probabilistic forecasts.
# - REFACTOR: Core prediction functions were refactored to generate and return these distributions.
#   Regression models (SARIMA, LSTM) now create a discretized normal distribution around their forecast.
# - HELPER: Added `get_best_guess_set` to derive a unique set of numbers from the distributions' modes.
# ======================================================================================================
...
st.set_page_config(
    page_title="LottoSphere v17.0.0: Quantum Chronodynamics",
    page_icon="⚛️",
    layout="wide",
)
...
def get_best_guess_set(distributions: List[Dict[int, float]], max_nums: List[int]) -> List[int]:
    ...
    for i, guess in enumerate(best_guesses):
        ...
        while candidate in seen:
            # If guess is taken, pick the next most likely, and so on.
            sorted_dist = sorted(distributions[i].items(), key=lambda item: item[1], reverse=True)
            for num, prob in sorted_dist:
                if num not in seen:
                    candidate = num
                    break
            # If all are taken (highly unlikely), pick a random valid number
...
def analyze_stable_position_dynamics(_df: pd.DataFrame, position: str, max_num: int) -> Dict[str, Any]:
    ...
    try:
        ...
        # SARIMA with probabilistic output
...
def predict_torch_model(_df: pd.DataFrame, _model_cache: Tuple, model_type: str, seq_length: int, max_nums: List[int]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Use training loss as a proxy for prediction variance
        std_dev = max(1.5, np.sqrt(best_loss) * (max_nums[0] - 1)) # Scaled std dev
        ...
        for name, func in model_funcs.items():
            ...
            for i in range(len(val_df)):
                ...
                for pos_idx, dist in enumerate(pred_obj['distributions']):
                    ...
                    prob_of_true = dist.get(true_num, 1e-9) # Use small epsilon for zero probability
...
st.title("⚛️ LottoSphere v17.0.0: Quantum Chronodynamics Engine")
    ...
    if not df_master.empty and df_master.shape[1] == 6:
        ...
        with tab1:
            ...
            with st.expander("Explanation of Probabilistic Forecasts"):
                st.markdown("""
                This panel now provides **full probability distributions** for each number position, offering deeper insights than single-number predictions.
                - **Backtesting with Log Loss**: Models are now scored using **Logarithmic Loss**, a rigorous metric that evaluates the accuracy of the assigned probabilities. A lower score is better.
                - **Likelihood Score**: This score is now based on Log Loss. A higher score indicates a model that is both accurate and confident about its predictions.
                - **Candidate Set**: This is the set of the **most likely numbers** (the mode) from each position's distribution, with duplicates resolved intelligently.
                - **Visualizations**: Click on any model's result to see the full probability distribution for each position. High, sharp peaks indicate high confidence; flat distributions indicate uncertainty.
                """)
        ...
        with tab2:
            ...
            if st.button(f"Analyze Dynamics for {position}", use_container_width=True):
                # UI for tab2 remains largely the same, focusing on dynamics not prediction
                st.info("This tab remains unchanged, providing analysis of the system's underlying dynamics.")
